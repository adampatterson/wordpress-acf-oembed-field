...
class acf_field_oembed_object extends acf_field {
	...
	*/
	...
	function __construct() {
		...
		$this->name = 'oembed_object';
		$this->label = __('oEmbed Object', 'acf-oembed_object');
	...
	*/
	...
	function format_value( $value, $post_id, $field ) {
		...
		$standard = $this->wp_oembed_get($value, $field['width'], $field['height']);
		$object =  $this->get_oembed_object($value, $dimensions);

		return [
			'html'				=> $standard,
			'title'				=> $object->title,
			'provider_name' 	=> $object->provider_name,
			'provider_url'		=> $object->provider_url,
			'author_name'		=> $object->author_name,
			'author_url'		=> $object->author_url,
			'thumbnail_url'		=> $object->thumbnail_url,
			'thumbnail_width'	=> $object->thumbnail_width,
			'thumbnail_height'	=> $object->thumbnail_height,
		];
